function cycleVSs(dataSets,dataTypes, savenum)
...
%             comments = cell2mat(params.comments);
            saveFilename = [saveFolder filesep sprintf('%s %s %s Vector Strength stats.xlsx',dataSet,cellType,dataType)];
...
%         end
        ...
        [figVS,VSArray,figMU,muArray,labels] = plotCycleVSs(dataCell,cellType,dataType,saveFilename); % plot figure for each cell type
        titleString = sprintf('%s %s %s Vector Strength',dataSet,cellType,dataType);
        title(gca(figVS),titleString,'FontSize',titlefontsize,'Fontweight','bold');
        titleString = sprintf('%s %s %s Vector Strength Phase',dataSet,cellType,dataType);
        title(gca(figMU),titleString,'FontSize',titlefontsize,'Fontweight','bold');  

        if savenum
            saveFilename = [saveFolder filesep sprintf('%s %s %s Vector Strength.svg',dataSet,cellType,dataType)];
            print(figVS,'-vector','-dsvg',saveFilename);
            saveFilename = [saveFolder filesep sprintf('%s %s %s Vector Strength Phase.svg',dataSet,cellType,dataType)];
            print(figMU,'-vector','-dsvg',saveFilename);
        end
    ...
    if nCellTypes > 2
        comments = cell2mat(params.comments);
        saveFilename = [saveFolder filesep sprintf('%s %s Spike 1 Vector Strength stats.xlsx',dataSet,dataType)];
