...
for iID = 1:nIDs
    ...
    if ~isempty(file.cell.CWTmaxValues) && ~isempty(file.lfp.CWTmaxValues)
        xArray(iID) = log10(file.cell.CWTmaxValues(3)); % cell peak scalogram power
        yArray(iID) = log10(file.lfp.CWTmaxValues(3)); % lfp peak scalogram power
    else
        xArray(iID) = NaN; % ignore if either value is empty
        yArray(iID) = NaN;
    end
...
xlim(limits);
ylim(limits);
xlabel('CaMK2 Inhibition Peak Gamma (Log pA^{2})','FontSize',12,'FontWeight','bold');
ylabel('CaMK2 LFP Peak Gamma Power (Log uV^{2})','FontSize',12,'FontWeight','bold');
