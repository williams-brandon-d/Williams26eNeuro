function plotCamk2inhibitionVsLFPfreq()
...
for iID = 1:nIDs
    ...
    if ~isempty(file.cell.CWTmaxValues) && ~isempty(file.lfp.CWTmaxValues)
        ...
        yArray(iID) = file.lfp.CWTmaxValues(2); % lfp peak scalogram freq
    else
        xArray(iID) = NaN; % ignore if either value is empty
        yArray(iID) = NaN;
    end
...
xlabel('CaMK2 Inhibition Peak Gamma Freq (Hz)','FontSize',12,'FontWeight','bold');
ylabel('CaMK2 LFP Peak Gamma Freq (Hz)','FontSize',12,'FontWeight','bold');
...
if savenum
    ...
    saveFilename = [saveFolder filesep 'camk2 inhibition vs lfp freq.svg'];
